...
```{r clean, results='hide', hide =TRUE}
rm(list = ls())
```
...
I also examined descriptives of the ego network and of the whole collaboration network of scholars. Among others, these included the number of nodes, edges, reciprocity, degreee centrality, clustering, density, and path-length. **[INSERT RELEVANT DESCRIPTIVES HERE]**\
\
Then, the networks were visualized using graphs to identify the presence of clustering and examine whether they are concentrated among individuals with the highest h-index or around specifically gendered scholars.

**[UNSURE IF THIS IS NEEDED IF GENDER IS FIRST COVARIATE AND H INDEX ONLY CONTRL VARIABLE]**

Various centrality measures were also calculated and analyzed to determine their correlation with the scholars' h-index, providing further insight into whether network hubs tend to form around those with the highest h-index.
...
```{r gender per dep., hide = TRUE, echo=FALSE }
...
```{r summary h index, hide = TRUE, echo=FALSE }
summary(df_ego$h_index)
```

H-Index Whole sample

```{r hist h index, hide = TRUE, echo=FALSE }
...
```{r table per dep h index, hide = TRUE, echo=FALSE }
